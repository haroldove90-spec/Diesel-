...
export const ClienteView: React.FC<ClienteViewProps> = () => {
  ...
  if (!currentOrder) {
    return (
      <div className="flex-1 p-8 text-center text-slate-500 bg-slate-50">
  ...
  return (
    <div className="flex-1 bg-slate-50 p-4 md:p-8 overflow-y-auto min-h-0 space-y-6 max-w-5xl mx-auto w-full">
      {/* Top Banner: Token Selector Simulator for Web Portal */}
      <div className="p-3.5 bg-white border border-blue-200 rounded-xl flex flex-wrap items-center justify-between gap-3 text-xs shadow-sm">
        <div className="flex items-center gap-2 text-slate-700 font-medium">
          <ExternalLink className="w-4 h-4 text-blue-600" />
          <span>Acceso por Enlace Único (Sin inicio de sesión):</span>
          <span className="font-mono text-blue-800 font-bold">taller.diesel/track/{activeToken}</span>
        </div>

        <div className="flex items-center gap-2">
          <label className="text-[10px] text-slate-600 font-bold uppercase">Cambiar Folio de Prueba:</label>
          <select
            value={activeToken}
            onChange={(e) => setActiveToken(e.target.value)}
            className="bg-white border border-slate-300 text-xs text-slate-900 p-1.5 rounded-md font-mono"
          >
            {orders.map(o => (
      ...
      <div className="bg-white border border-slate-200 p-6 rounded-xl space-y-4 shadow-sm">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 border-b border-slate-200 pb-4">
          <div>
            <span className="text-[10px] font-black text-[#002855] uppercase tracking-widest block mb-1">
              Portal de Tracking de Unidad Diesel • TSR SONORA
            </span>
            <h1 className="text-xl font-bold text-slate-900">
              {currentOrder.vehicle.brand} {currentOrder.vehicle.model} ({currentOrder.vehicle.year || '2021'})
            </h1>
            <p className="text-xs text-slate-600 font-mono mt-0.5">
              Placas: <strong className="text-slate-900">{currentOrder.vehicle.plates}</strong> • VIN: {currentOrder.vehicle.vin}
            </p>
          </div>

          <div className="text-left sm:text-right">
            <span className="text-[10px] text-slate-500 uppercase font-bold block">Propietario / Empresa</span>
            <span className="text-xs font-bold text-slate-900">{currentOrder.vehicle.clientName}</span>
          </div>
        </div>
        ...
        <div className="space-y-3 pt-2">
          <div className="flex justify-between items-center text-xs">
            <span className="text-slate-600 font-bold uppercase">Estado Actual del Servicio:</span>
            <span className="font-bold text-blue-800 font-mono uppercase bg-blue-50 px-2.5 py-1 rounded-md border border-blue-200">
              {currentOrder.status}
            </span>
          </div>
        ...
        {currentOrder.evidences.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {currentOrder.evidences.map((ev) => (
              <div key={ev.id} className="bg-slate-50 border border-slate-200 rounded-lg overflow-hidden">
      ...
      <div className="bg-white border border-slate-200 p-6 rounded-xl space-y-5 shadow-sm">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-slate-200 pb-3 gap-2">
          ...
          <div className="text-right">
            <span className={`status-pill ${
              currentOrder.clientApproved ? 'bg-emerald-100 text-emerald-800' :
              currentOrder.clientApproved === false ? 'bg-red-100 text-red-800' : 'bg-amber-100 text-amber-800'
            }`}>
        ...
        <div className="space-y-3">
          <p className="text-[10px] font-bold text-slate-600 uppercase tracking-wider">Refacciones Requeridas:</p>
          <div className="space-y-1.5">
            {currentOrder.parts.map((p, idx) => (
              <div key={idx} className="flex justify-between items-center p-3 bg-slate-50 border border-slate-200 rounded-lg text-xs">
                <div>
                ...
                <span className="font-mono text-blue-800 font-bold">${(p.quantity * p.unitPrice).toLocaleString('es-MX')}</span>
              </div>
            ))}
          </div>

          <p className="text-[10px] font-bold text-slate-600 uppercase tracking-wider pt-2">Mano de Obra y Diagnóstico:</p>
          <div className="space-y-1.5">
            {currentOrder.labor.map((l, idx) => (
              <div key={idx} className="flex justify-between items-center p-3 bg-slate-50 border border-slate-200 rounded-lg text-xs">
                <div>
                  <p className="text-slate-900 font-bold">{l.description}</p>
                  <p className="text-[10px] text-slate-500 font-mono">Tiempo estimado: {l.hours} hrs</p>
                </div>
                <span className="font-mono text-blue-600 font-bold">${(l.hours * l.hourlyRate).toLocaleString('es-MX')}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="pt-4 border-t border-slate-200 flex flex-col sm:flex-row justify-between items-center gap-4">
          <div>
            <p className="text-[10px] text-slate-500 uppercase font-bold">Total General Cotizado:</p>
            <p className="text-2xl font-mono text-blue-900 font-bold mt-0.5">
          ...
          <div className="flex gap-3 w-full sm:w-auto">
            <button
              ...
              className="flex-1 sm:flex-none flex items-center justify-center gap-2 border border-red-300 text-red-700 hover:bg-red-50 px-5 py-2.5 rounded-md text-xs font-bold uppercase transition-all cursor-pointer shadow-sm"
            >
              <X className="w-4 h-4" />
            ...
            <button
              ...
              className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-2.5 rounded-md text-xs font-bold uppercase shadow-sm transition-all cursor-pointer"
            >
